...
////////////////////////////////////////////////////////////////
	...
	CanvasRenderer.prototype.resize = function(width, height) {
		...
		if(this.height < this.width * 0.3) this.height = Math.floor(this.width * 0.3);
		...
		this.whiteKeyHeight = Math.floor(this.height * 0.6);
		this.blackKeyWidth = Math.floor(this.whiteKeyWidth * 0.65);
		this.blackKeyHeight = Math.floor(this.whiteKeyHeight * 0.6);
		this.blackKeyOffset = Math.floor(this.whiteKeyWidth - (this.blackKeyWidth / 2));
		this.keyMovement = Math.floor(this.whiteKeyHeight * 0.02);
		
		// Calculate blip dimensions (note indicators)
		...
		// Piano frame dimensions
		this.frameHeight = Math.floor(this.height * 0.15);
		this.frameY = this.whiteKeyHeight;
		
		// Prerender white key
		this.whiteKeyRender = document.createElement("canvas");
		this.whiteKeyRender.width = this.whiteKeyWidth + 10;
		this.whiteKeyRender.height = this.whiteKeyHeight + 10;
		...
		if(ctx.createLinearGradient) {
			...
			gradient.addColorStop(0.97, "#f0f0f0");
		...
		// White key border and fill
		ctx.strokeStyle = "#aaaaaa";
		ctx.lineWidth = 1;
		
		// Draw white key with rounded bottom corners
		const radius = 4;
		ctx.beginPath();
		ctx.moveTo(1, 1);
		ctx.lineTo(1, this.whiteKeyHeight - radius);
		ctx.arcTo(1, this.whiteKeyHeight, 1 + radius, this.whiteKeyHeight, radius);
		ctx.lineTo(this.whiteKeyWidth - radius, this.whiteKeyHeight);
		ctx.arcTo(this.whiteKeyWidth, this.whiteKeyHeight, this.whiteKeyWidth, this.whiteKeyHeight - radius, radius);
		ctx.lineTo(this.whiteKeyWidth, 1);
		ctx.lineTo(1, 1);
		ctx.closePath();
		ctx.fill();
		ctx.stroke();
		
		// Add side gradient for 3D effect
		if(ctx.createLinearGradient) {
			var sideGradient = ctx.createLinearGradient(0, 0, this.whiteKeyWidth, 0);
			sideGradient.addColorStop(0, "rgba(0,0,0,0.1)");
			sideGradient.addColorStop(0.3, "rgba(255,255,255,0)");
			sideGradient.addColorStop(0.7, "rgba(255,255,255,0)");
			sideGradient.addColorStop(1, "rgba(0,0,0,0.1)");
			ctx.fillStyle = sideGradient;
			ctx.fill();
		}
		
		// Add gloss/reflection effect
		if(ctx.createLinearGradient) {
			var glossGradient = ctx.createLinearGradient(0, 0, 0, this.whiteKeyHeight * 0.5);
			glossGradient.addColorStop(0, "rgba(255,255,255,0.6)");
			glossGradient.addColorStop(1, "rgba(255,255,255,0)");
			ctx.fillStyle = glossGradient;
			ctx.beginPath();
			ctx.rect(2, 2, this.whiteKeyWidth - 3, this.whiteKeyHeight * 0.5);
			ctx.fill();
		}
		...
		ctx.beginPath();
		ctx.moveTo(1, 1);
		ctx.lineTo(1, this.blackKeyHeight - blackRadius);
		ctx.arcTo(1, this.blackKeyHeight, 1 + blackRadius, this.blackKeyHeight, blackRadius);
		ctx.lineTo(this.blackKeyWidth - blackRadius, this.blackKeyHeight);
		ctx.arcTo(this.blackKeyWidth, this.blackKeyHeight, this.blackKeyWidth, this.blackKeyHeight - blackRadius, blackRadius);
		ctx.lineTo(this.blackKeyWidth, 1);
		ctx.lineTo(1, 1);
		ctx.closePath();
		ctx.fill();
		ctx.stroke();
		...
		// Add side gradient for 3D effect
		if(ctx.createLinearGradient) {
			var sideGradient = ctx.createLinearGradient(0, 0, this.blackKeyWidth, 0);
			sideGradient.addColorStop(0, "rgba(0,0,0,0.9)");
			sideGradient.addColorStop(0.4, "rgba(0,0,0,0.5)");
			sideGradient.addColorStop(0.6, "rgba(0,0,0,0.5)");
			sideGradient.addColorStop(1, "rgba(0,0,0,0.9)");
			ctx.fillStyle = sideGradient;
			ctx.fill();
		}
		
		// Add subtle gloss/reflection effect
		if(ctx.createLinearGradient) {
			var glossGradient = ctx.createLinearGradient(0, 0, 0, this.blackKeyHeight * 0.4);
			glossGradient.addColorStop(0, "rgba(255,255,255,0.15)");
			glossGradient.addColorStop(1, "rgba(255,255,255,0)");
			ctx.fillStyle = glossGradient;
			ctx.beginPath();
			ctx.rect(2, 2, this.blackKeyWidth - 3, this.blackKeyHeight * 0.4);
			ctx.fill();
		}
		
		// Prerender piano frame
		this.frameRender = document.createElement("canvas");
		this.frameRender.width = this.width;
		this.frameRender.height = this.frameHeight + 10;
		var ctx = this.frameRender.getContext("2d");
		
		// Rich wooden frame
		if(ctx.createLinearGradient) {
			var frameGradient = ctx.createLinearGradient(0, 0, 0, this.frameHeight);
			frameGradient.addColorStop(0, "#5c3c25");
			frameGradient.addColorStop(0.5, "#8b572a");
			frameGradient.addColorStop(1, "#4a3020");
			ctx.fillStyle = frameGradient;
		} else {
			ctx.fillStyle = "#8b572a";
		}
		
		ctx.fillRect(0, 0, this.width, this.frameHeight);
		...
		if(ctx.createLinearGradient) {
			for(let i = 0; i < 8; i++) {
				...
				var woodGrain = ctx.createLinearGradient(x, 0, x + width, this.frameHeight);
				woodGrain.addColorStop(0, "rgba(255, 255, 255, 0)");
				woodGrain.addColorStop(0.5, "rgba(255, 255, 255, 0.03)");
				woodGrain.addColorStop(1, "rgba(255, 255, 255, 0)");
				
				ctx.fillStyle = woodGrain;
				ctx.fillRect(x, 0, width, this.frameHeight);
			}
		}
		...
		// Bottom shadow
		if(ctx.createLinearGradient) {
			var shadowGradient = ctx.createLinearGradient(0, this.frameHeight - 8, 0, this.frameHeight);
			shadowGradient.addColorStop(0, "rgba(0, 0, 0, 0)");
			shadowGradient.addColorStop(1, "rgba(0, 0, 0, 0.3)");
			ctx.fillStyle = shadowGradient;
			ctx.fillRect(0, this.frameHeight - 8, this.width, 8);
		}
		...
		for(var j = 0; j < 2; j++) {
			...
			ctx.shadowBlur = this.keyMovement * 4;
		...
		// Prerender blip (note indicator)
		this.whiteBlipRender = document.createElement("canvas");
		this.whiteBlipRender.width = this.whiteBlipWidth + 10;
		this.whiteBlipRender.height = this.whiteBlipHeight + 10;
		var ctx = this.whiteBlipRender.getContext("2d");
		if(ctx.createRadialGradient) {
			var gradient = ctx.createRadialGradient(
				this.whiteBlipWidth/2, this.whiteBlipHeight/2, 0,
				this.whiteBlipWidth/2, this.whiteBlipHeight/2, this.whiteBlipWidth/2
			);
			gradient.addColorStop(0, "rgba(100, 200, 255, 0.9)");
			gradient.addColorStop(1, "rgba(50, 150, 220, 0.3)");
			ctx.fillStyle = gradient;
		} else {
			ctx.fillStyle = "rgba(100, 200, 255, 0.5)";
		}
		ctx.beginPath();
		ctx.arc(this.whiteBlipWidth/2, this.whiteBlipHeight/2, this.whiteBlipWidth/2, 0, 2 * Math.PI);
		ctx.fill();
		
		this.blackBlipRender = document.createElement("canvas");
		this.blackBlipRender.width = this.blackBlipWidth + 10;
		this.blackBlipRender.height = this.blackBlipHeight + 10;
		var ctx = this.blackBlipRender.getContext("2d");
		if(ctx.createRadialGradient) {
			var gradient = ctx.createRadialGradient(
				this.blackBlipWidth/2, this.blackBlipHeight/2, 0,
				this.blackBlipWidth/2, this.blackBlipHeight/2, this.blackBlipWidth/2
			);
			gradient.addColorStop(0, "rgba(170, 255, 230, 0.9)");
			gradient.addColorStop(1, "rgba(100, 230, 200, 0.3)");
			ctx.fillStyle = gradient;
		} else {
			ctx.fillStyle = "rgba(170, 255, 230, 0.5)";
		}
		ctx.beginPath();
		ctx.arc(this.blackBlipWidth/2, this.blackBlipHeight/2, this.blackBlipWidth/2, 0, 2 * Math.PI);
		ctx.fill();
	...
	// Add this method to render the piano frame
	CanvasRenderer.prototype.renderPianoFrame = function(ctx) {
		ctx.drawImage(this.frameRender, 0, this.frameY);
	};
